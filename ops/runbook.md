...
| `fomo-narrative-source-scan` | 00/06/12/18 UTC (probation live posts; `narrative_source_probation`) |
| `narrative-source-review` | daily 19:00 UTC (promote/demote + gated follow) |
...
reloads units, then clears the pause and kickstarts deferred jobs;
...
- Router health: `curl -sS http://127.0.0.1:8787/healthz` must return
  ...
  research needs `DISCORD_RESEARCH_BOT_TOKEN` when `chat.discord.enabled`; desk
  pull needs `DESK_PULL_TOKEN`; optional Grok webhook needs both `INTAKE_*`
...
- Incident remediation (schema 17+ / ADR 017+025+027+028+030): hourly/weekly host
...
- **Daily Telegram digest** — VPS systemd timer `trenchcoat-job-telegram-digest`
  ...
  section. Distiller aims for ~8000 characters. Longer maps still send.
  The host does not send a `.md` file to the operator bot (ADR 052).
  Manual:
