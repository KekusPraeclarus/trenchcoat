...
# Usage: ops/install-launchd.sh [--dry-run] [--with-harness] [--jobs-only] [--no-load] [--sync-env]
...
<dict>
  ...
  <array>
    <string>/bin/sh</string>
    <string>-c</string>
    <string>$WRAPPER_PREFIX; exec $TC listen telegram</string>
  </array>
  <key>KeepAlive</key>
  <true/>
...
echo "listener keepalive: launchctl print $DOMAIN/com.trenchcoat.listener"
