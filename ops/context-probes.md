...
| P78 | recall | When two Playwright sessions fail auth, where does the warning go, and does one X hold send it? | Operator Telegram DM via `telegramSendOperatorMessageChunks` (interface bot). Never the router. Threshold is 2. One open issue does not send. → INV-R6, telegram.md, `auth-issue-notify.ts` | pass | 2026-08-31 |
...
| P82 | recall | Where must Grok intake keys live, and how does the desk poll without Telegram? | Pull-queue: `DESK_PULL_TOKEN` in `~/.trenchcoat/env`, loopback `:8788`, public HTTPS only `/desk/intake/*`. Optional webhook: both `INTAKE_*` keys in the same env file. Restart the router after a write. Webhook ping is `POST {"ping":true}` to the webhook only — never `/v1/events` → ADR 051, ADR 050, router.md, ops/desk-pull.md | pass | 2026-09-06 |
