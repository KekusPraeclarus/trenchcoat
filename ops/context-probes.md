...
|----|------|----------|--------------------------|-------------|------|
| P1 | recall | How do Telegram and Discord market broadcasts relate after channel render? | Same rendered text on both when `channels.telegram` is set; optional Grok gets a JSON twin of that text (ADR 050). Topic-merged followers omit all three. No separate Discord daily budget (ADR 041) → router.md, orchestrator.md, INV-B2/B4 | pass | 2026-09-04 |
...
| P3 | artifact | Where does Farcaster (Neynar) live, and what does ops enablement still need? | Implemented under `src/collectors/farcaster/` with `farcaster-scan` / `fc-source-review` (ADR 007); enable via `farcaster.enabled` + Neynar/signer auth → collectors.md, TECHNICAL-SPEC, knowledge/neynar.md | pass | 2026-09-04 |
| P4 | continuation | What is the offline vs live acceptance status of the implementation? | Offline suites green. Phase 0–3 DONE 2026-07-18. No numbered Phase 4. Residual live work in `ops/LIVE-E2E-BLOCKERS.md` (also root README Status) | pass | 2026-09-04 |
| P24 | continuation | What is next after Phase 3 of the 2026-07-18 audit roadmap? | Phase 3 DONE. No numbered Phase 4. Further work is remaining PARTIAL invariants and live ops → root README Status + `ops/LIVE-E2E-BLOCKERS.md` | pass | 2026-09-04 |
| P27 | recall | Does the host Cursor sandbox block reads and writes outside `agent/` equally? | Writes outside are blocked (`--sandbox enabled` + `disableTmpWrite: true`); outside reads still succeed on current CLI — INV-I1 PARTIAL; never rely on tmpdir-only escape probes → knowledge/cursor-cli.md, agent-workspace.md, INV-I1 | pass | 2026-08-18 |
| P7 | recall | How does trenchcoat authenticate Cursor agent job sessions? | Cursor CLI login (`agent login` / `agent status`), headless `agent -p --trust --workspace agent/`; not `@cursor/sdk` / required `CURSOR_API_KEY` → ADR 003, docs/knowledge/cursor-cli.md | pass | 2026-08-18 |
...
| P9 | recall | Who may add or remove members of the bot-managed X list, and from what evidence? | Only host lifecycle code after lagged settled direct bullish raw-CA outcomes; FYP text/model/engagement cannot promote; operator lists are immutable inputs → ADR 004, source-lifecycle.md, INV-S21 (PARTIAL until sealed outcomes feed review) | pass | 2026-08-18 |
| P10 | recall | Which X network mutations are allowed, and what must match before any membership change? | Only GraphQL `CreateList`/`ListAddMember`/`ListRemoveMember` in the host synchronizer; target list id must equal persisted managed list id; scrapers stay read-only except allowlisted timeline GraphQL POSTs (mutations stay blocked) → INV-R2 (PARTIAL — allowlists ENFORCED; like throttle config-default), knowledge/x-playwright.md. FOMO platform follow is a separate INV-R2 exception (`/follows` + Statsig initialize), not an X mutation | pass | 2026-09-04 |
...
| P12 | recall | Can the agent like FYP posts, and does that promote managed-list membership? | Agent owns like/follow choices; likes must target same-run FYP post ids; host also follows an FYP author after 25 unique liked posts (`follow_after_likes`); proposal-time subscription dedupe (`already_liked` / `already_following` / `not_following` / `pending_duplicate`); default ≤2 likes / 10 min (INV-S22 PARTIAL); engagement never writes managed-list or source scores → INV-S22, source-lifecycle.md, x-playwright.md | pass | 2026-09-08 |
...
| P52 | recall | May the self-improvement loop improve itself (harness code/gates), and what is the bounded config alternative? | No autonomous edit of `src/harness/**`, evaluator, confinement, or allowlist. Default target stays `policy.json`. Hand-shipped mining/manifesto/keep/prior-attempts are implemented. Bounded `config/harness-improver.json` is ADR 039: shadow trials + host meta-utility (≥8 pairs) + operator-only `tc harness meta promote` → ADR 038/039, INV-S24, harness-self-improvement-patterns.md | pass | 2026-07-27 |
| P53 | recall | What sealed inputs may harness propose/mining/keep read, and where do numeric decision outcomes live? | Sealed aggregates, decision metadata/enums, numeric archived signals, host gate flags, settled numeric outcomes only — never inbox/scraped prose. Outcomes at `archive/outcomes/decision/<id>/<h>h.json` via `runSettleDecisions`; mining/keep/canary consume them → INV-S24, snapshot-archive.md, audit-metrics.md, harness-improvement.md | pass | 2026-07-27 |
| P54 | recall | What does manifesto validation fail closed on, and which plan fields does AHE-style observability require? | Plan v2: `evidenceIds`, `rootCauseHypothesis`, `predictedFixes`, `atRiskRegressions`. `manifesto-validation.json` fails on unpredicted protected-metric regression; prediction misses without regression are recorded but do not hard-fail primary gates → harness-improvement.md, `manifesto-validate.ts` | pass | 2026-07-27 |
| P55 | recall | Where is the prior-attempts negative-result index, and can it be rebuilt? | Host `~/.trenchcoat/harness-improvements/prior-attempts.jsonl` (+ per-hypothesis `prior-attempts-summary.json`); rebuildable from rejection/rollback receipts — never scraped text → harness-improvement.md, `prior-attempts.ts`, INV-S24 | pass | 2026-07-27 |
| P56 | recall | What are the meta allowlist, utility pass rule, and promote path for improver-config? | Literal allowlist `config/harness-improver.json` only. `promotion_eligible` needs ≥8 valid pairs, ≥5 candidate wins, win rate > baseline, protected/invalid counts no worse, median primary improvement >0 and no worse than baseline, safety ok. Schedule is shadow-only; first eligibility sends one-shot Telegram operator briefing (`operator-notify.json`); `tc harness meta promote` revalidates then ff-integrates. Job `harness-meta-improve` is INV-S15 lock-exempt (ADR 027) → ADR 039, harness-improvement.md, CONFIG.md | pass | 2026-07-27 |
| P32 | recall | After a config schema bump, which three sites must stay aligned for deploy provenance, and where is the previous runtime kept on rollback? | `ConfigSchema`/migrations, `DEPLOYMENT_CONFIG_SCHEMA` (`deployment.ts` / health `expectedSchema`), and the host install script `configSchema` (`install-systemd.sh` on Linux, `install-launchd.sh` on macOS) must match; previous runtime is `~/.trenchcoat/runtime.prev` (not `runtime.previous`) → orchestrator.md Deployment manifest, runbook.md, linux-vps.md | pass | 2026-08-18 |
...
| P57 | recall | How does a readiness deferral surface, and how is harness kicked after audit without touching audit sealing? | `assessHarnessImproveReadiness` typed skip (`reasonSlug`/`nextAction`) persists `harness-improvements/schedule-report.json` + skip ledger; health classifies host-report `skipped` as lastSkip not lastSuccess; `tc harness status` shows readiness. Install scripts only: success-exit audit kicks independent harness service/timer non-blocking; weekly timer remains fallback; no audit code/seal path change → harness-improvement.md, orchestrator.md, INV-S15/S24 | pass | 2026-08-06 |
| P60 | recall | What must a narrative claim show before it can stage, and does curation change the archive? | `curateSocialEvidence` filters only the derived `narrative-social-*` snapshots (`collector-status`, `expired`, `duplicate` by dedupeKey/URL/text hash, `promotion-pattern`, `repeated-promotion`); `assessNarrativeEvidenceQuality` needs `min_fresh_posts` posts, `min_independent_authors` authors, promotional share ≤ `max_promotional_share` for tier `strong`; `ingestOutbox` rejects narrative claim types below strong as `narrative-evidence-quality:<floor>`; primary handles add signal only; sealed collector archives stay byte-identical → ADR 042, collectors.md, INV-B2 | pass | 2026-08-10 |
| P61 | recall | Whose Discord reactions count as broadcast feedback, and what may a tuning candidate change? | Only `👍`/`👎` from `DISCORD_OPERATOR_USER_ID` in `broadcast.feedback.channel_id`; `provider_message_index` maps the message id back to the delivery/event, and any part of a split broadcast maps to one `feedbackId` (repeat reactions are no-ops). Both marks = `ambiguous`, no marks = `retracted`; `down`/`ambiguous` open one 72h Telegram request whose raw prose stays in a confined `untrusted-external` evidence file — only bounded tags plus one derived summary reach the ledger. `broadcast feedback candidate` may write only `agent/skills/decision-policy/policy.json` and `config/broadcast-output-tuning.json`, weight Δ ≤ 0.25, threshold Δ ≤ 0.10, ≤ 4 rules, +0.10 development agreement, no holdout regression, one unused market holdout with protected metrics passing; apply needs a clean repo + `repo-mutation.lock` and never commits or deploys. Harness reads only `sealed/active-preference-set.json` → ADR 043, broadcast-feedback.md, INV-B3/B6/S24 | pass | 2026-08-10 |
| P62 | recall | Where does narrative prose memory live, when is it written, and what cleans up alpha-ack tombstones? | `state/narratives/<slug>.md` dossiers: narrative-scan writes them only on broadcast triggers; host marks `status: dormant` on log prune (`markNarrativeDossierDormant`); retention deletes dossiers untouched past `retention.narrative_dossier_days` whose slug left the log. Alpha acks live in `state/alpha-acks/` (never `state/research/`); `retainAlphaAckTombstones` sweeps them after purge + `retention.alpha_ack_days`; archived digest receipt stays the durable record → ADR 044/045, agent-workspace.md, INV-Q2 | pass | 2026-08-10 |
| P14 | recall | Who may write `state/wallets.json`, and how do wallet lifecycle events relate to market broadcast fanout? | Host-only (discovery/scan/review/seed); evidence-only `wallet-evidence` agent may write advisory `wallet-evidence.md` only — never state/scores/cursors/lifecycle; each applied transition emits one `wallet.lifecycle` router event that skips channel render → smart-wallets.md, ADR 002, INV-S19/S20 | pass | 2026-09-04 |
...
| P15 | recall | What are the two “outbox” surfaces, and which module stages router delivery? | `agent/outbox/<run-id>.json` = BroadcastItem proposals; host `ingestOutbox` validates/stages → `archive/router-outbox/` via `src/lib/outbox.ts`; then `renderChannelPayloads` (Telegram topic render; Discord forwards same text) then `deliverStagedOutbox`. No `src/orchestrator/outbox.ts`. HMAC in `src/orchestrator/router.ts` → orchestrator.md "Outbox → router", INV-B2 | pass | 2026-08-31 |
...
| P70 | recall | Why can wallet scans starve `outcomes-settle` / `wallet-review`, and what fixes it? How do paper track positions leave `entry-pending`? | Pre-ADR 031 scans held `agent/.lock` for Cursor+Helius; settle/review exited 3. Fix: those jobs + wallet-scan are lock-exempt with brief RMW only; scans host-own `wallets.json` and may still run an advisory `wallet-evidence` session (ADR 002). Cap `max_wallets_per_scan`. Entries finalise via host `settle-ledger` in `outcomes-settle` (decisionTs + first post-decision bar); drop on unfilled → `censored`. If brief RMW still cannot get the lock, settle records `lockDeferred` and completes. Never hand-edit `ledger.json`. → ADR 031, smart-wallets.md, INV-S10/S15 | pass | 2026-08-18 |
| P58 | recall | When would Discord omit a market broadcast that Telegram received? | Only when `channels.telegram` is omitted (`topic-merged` follower). Receipt `discord: topic-merged`. No separate Discord budget (ADR 041) → router.md, channel-render-receipts | pass | 2026-08-18 |
...
| P19 | recall | Must the router process be running for broadcasts to reach Telegram/Discord, and how is it scheduled? | Yes — jobs only stage + HMAC-POST; fanout is the KeepAlive `com.trenchcoat.router` (`tc router serve`) via `install-launchd.sh`. Needs `TRENCHCOAT_ROUTER_*` + destination env (`TELEGRAM_ROUTER_BOT_TOKEN`/`CHAT_ID` and/or `DISCORD_WEBHOOK_URL`). Optional Grok needs both `INTAKE_*` keys in `~/.trenchcoat/env` (ADR 050). Runtime install rebuilds `better-sqlite3`. Bare intake URL defaults to `/v1/events`; loopback HTTP allowed → router.md, runbook.md | pass | 2026-09-04 |
| P20 | recall | What text does Telegram vs Discord get for a market broadcast, and where do the rewrites run? | One host render path: fail-closed topic paragraph from bounded topic packet when `broadcast.telegram_overview.enabled` (`distill-session.ts` / `renderChannelPayloads`; ≤800 chars; else short `event.text`). Discord copies the same `channels.telegram.text` (`forwarded`). Grok gets a structured twin of that text (ADR 050). Topic-merged followers omit all three. `telegram_overview.daily_cap` caps LLM sessions (`topic-distill-<day>.json`). Daily `narrative.digest` is Telegram-only at 04:00 London (ADR 041). Distiller aims for ~8000 chars. The host does not send a raw `.md` to the operator bot (ADR 052) → router.md, INV-B2, ADR 049/050/052 | pass | 2026-09-15 |
...
| P28 | recall | How does Discord interactive research differ from Discord market broadcasts? | Two surfaces: router webhook `DISCORD_WEBHOOK_URL` (unified broadcast fanout per ADR 041) vs Gateway `DISCORD_RESEARCH_BOT_TOKEN` + `tc listen discord` (isolated `~/.trenchcoat/discord/`, ✅ start reaction then final-only text replies, INV-D1, ADR 010) → discord-research.md, knowledge/discord.md | pass | 2026-07-27 |
| P29 | recall | Can Fomo X-source classification directly add accounts to the managed X list or follow them? | No — host upserts linked-X nominations only; `fomo-x-source-review` returns strict JSON; merge fail-closes on missing evidence; shiller/`both` need X-post CAs + existing gates; narrative/`both` use 14-day probation before host X engagement. FOMO-platform follows are a separate host track (`fomo-follows.json`, ADR 048) → ADR 009/048, source-lifecycle.md, knowledge/fomo-family.md | pass | 2026-09-04 |
...
| P37 | continuation | What must happen before Fomo jobs can mutate the research queue or X nominations in production? | Burner `auth fomo` + live smoke (`pnpm fomo:smoke`) + `fomo:install-gates` with provider/capability `pass` (operator override or FAFO). Prefer 14-day shadow (`SHADOW-CANARY.md`) before `shadow_mode: false`. Profile `~/.trenchcoat/fomo-profile/` only. Fomo never writes `wallets.json`. `probe:fomo` is discover/status/sanitize scaffold — no evaluate yet → development.md, LIVE-E2E-BLOCKERS, knowledge/fomo-family.md, ADR 009/048 | pass | 2026-09-04 |
...
| P42 | recall | Why must channel copy not paste `72h`, and is `watchWindow` on the outbox proposal? | Host derives `watchWindow` at distill from claim type + `horizonHours` (ADR 013); settlement stays 24/72/168. Not agent-authored. Thin scrub replaces leaked hour tokens and rewrites weekly timeframes (`this week` → `if it holds`); other natural phrases stand. TG fanout is HTML + narrative deslug → watch-window.ts, telegram.md, INV-B2 | pass | 2026-07-20 |
...
| P46 | recall | Where do smart-wallet addresses come from, and what alert fires when tracked wallets converge on a fresh token? | On-chain verified buys only (Helius/Infura/Robinhood) — never Fomo profile addresses. Runner discovery qualifies fresh pools then recurrence ≥2/30d (`origin: new-pools`). ≥4 event-time `tracking` wallets → host-rendered `UNVERIFIED WALLET CONVERGENCE` + independent `wallet-convergence` research enqueue; defaults shadow/off until ADR 020 canary gates → smart-wallets.md, ADR 020, `ops/runner-wallet-canary.md`, INV-S19/S20 | pass | 2026-07-21 |
| P59 | recall | Where do Cursor token costs get cut without changing Discord message caps? | Batched x-scan (one list-scan/round), host alpha no-thesis ack, claim-only worthiness + 48h cache, distill `llm_budget_fraction` / hot-day fraction (ADR 034), review-reports-summary bullets, research-candidates-hint, chat turn_count_max — Discord no longer has a separate message budget (ADR 041) | pass | 2026-07-27 |
| P63 | recall | Which install script does remediation deploy/rollback use on the Linux VPS, and what fails if launchd is used? | `resolveHostInstallScript` → `ops/install-systemd.sh` on Linux (`install-launchd.sh` on macOS). Launchd on VPS fails with `launchctl: not found` and can halt remediation automation → incident-remediation.md § Repo root and deploy, ADR 017, `src/harness/deploy.ts` | pass | 2026-08-31 |
| P64 | recall | Why does `trenchcoat remediations run` fail with `wrong-branch` when cwd is a rem worktree? | CLI prefers cwd when `ops/install-launchd.sh` exists there. Rem worktrees include that file, so `assertCleanMain` runs against the rem branch. Run from `~/src/trenchcoat` or set `TRENCHCOAT_REPO_ROOT` → incident-remediation.md, linux-vps.md, `src/cli.ts` `cmdRemediations` | pass | 2026-08-12 |
| P65 | recall | What happens on pre-review `revise` vs `reject`, and what config caps auto-loops? | `revise` re-enters propose with `priorPreReviewPath` up to `max_pre_review_revises` (default 5), archives `proposal.revise-N.json` / `pre-review.revise-N.json`, then `pre-review-revise-exhausted:N`. `reject` fails on the first decision → incident-remediation.md, ADR 029, INV-S27, `src/remediation/pre-review-loop.ts` | pass | 2026-08-12 |
| P66 | recall | When a Discord conversation turn has a CA, how does the bot get live mcap/liquidity, and what must stay false? | Host `live-tape.ts` prefetches one gated DexScreener search after exactly one validated CA; injects trusted metrics into the ask prompt; soft-fails with no invented numbers; synthesis skips prefetch; `chain:CA` form does not fall back to bare CA → discord-conversation.md § Live market tape | pass | 2026-08-12 |
| P67 | recall | Can the pump-scan agent like or follow a coin that is not in the same-run eligible snapshot? | No. Likes bind to `pump-fyp-eligible` item ids. Follow/unfollow bind to authors in that snapshot (INV-S30). Following, leaderboard, and profile calls are evidence only → pump-fun.md, ADR 047 | pass | 2026-08-31 |
| P68 | recall | When does pump-scan scrape the Following tab? | Only when `state/pump-engagement.json` `followedHandles.length` is at least `following_min_follows` (10). Below that the host writes `following-skipped-below-min` → collectors.md, ADR 047 | pass | 2026-08-13 |
| P69 | recall | How does a Pump call settle, and what does a rug after the call do? | Wait `pump.calls.min_age_hours` (24h), then peak quiet 6h / force 14d / hit +20%. Security-gate `hardFail` writes `terminal-loss` `rugged-after-call` and stops. Scores use complete and terminal-loss only → ADR 047, settle-pump-calls.ts | pass | 2026-08-13 |
...
| P75 | continuation | What is the next step of the planned trading pipeline? | No trading pipeline exists or is planned in this repo. Trading lives outside this repo. INV-A1 holds. | pass | 2026-09-22 |
...
| P78 | recall | When a Playwright session fails auth, where does the warning go, and does one X hold send it? | Operator Telegram DM via `telegramSendOperatorMessageChunks` (interface bot). Never the router. Threshold is 1. Same open set does not send again. → INV-R6, telegram.md, `auth-issue-notify.ts` | pass | 2026-09-08 |
| P79 | recall | What is the current config schema, and did ADR 049 bump it? | Current schema is **29** (ADR 043 `history_days`). ADR 049 did not bump schema. `tc init` uses `migrateConfigToV29`. Triple-sync: ConfigSchema, DEPLOYMENT_CONFIG_SCHEMA, install scripts. → CONFIG.md, README setup | pass | 2026-09-04 |
| P80 | recall | Does Fomo register wallet candidates, and which feed is alerts? | Never wallets (INV-S19 / ADR 048). Alerts are `/feed/tradingActivity` only (INV-R7). TECHNICAL-SPEC + fomo-family.md | pass | 2026-09-04 |
...
- 2026-07-23 session-learning: ADR 031 (wallet settle/scan brief locks +
  `settle-ledger`); INV-S10/S15 cite 031; P57 added; gotchas empty. Deploy of
  031 still pending for live settle/review.

- 2026-07-27 context-maintenance: lint 3 errors fixed (gotchas path, broken links,
  ADR 038 numbering conflict). Renamed unified fanout to ADR 041; updated
  ARCHITECTURE broadcast boundary; probe duplicate IDs P50/P51 → P57/P58/P59;
  graded P1/P20/P28/P49/P52–P56/P57/P58 pass. Always-on ~833 tokens. Gotchas
  routed to `ops/gotchas.md`.

- 2026-07-23: ADR 034 token-cost host gates (batched x-scan, host alpha ack, claim-only worthiness cache, distill fractions, review bullets, research hint, chat turn caps). Added P59 (was misplaced P51).
- 2026-07-24: ADR 038 improver self-edit boundary (no harness-code RSI). Added P52.
- 2026-07-24: Self-Improving Harness Upgrade complete in docs — ADR 039 bounded
  improver-config lane (shadow + operator promote); mining/manifesto/keep/
  prior-attempts on policy lane; decision outcomes path for mining consumers;
  schema 21 `meta_*`; `harness-meta-improve` INV-S15 exempt (ADR 027). Updated
  P52; added P53–P56. Gotchas empty.

- 2026-08-12 session-learning + context-maintenance: Rem deploy used launchd on
  Linux (`launchctl: not found`) and halted automation; rem CLI cwd in a worktree
  caused `wrong-branch`. Documented `resolveHostInstallScript`, rem cwd rule,
  live-tape CA turns, and pre-review revise loop. Added P63–P66; refreshed P32.
- 2026-08-15 context-maintenance: lint 0 errors. INV-S6 demoted ENFORCED→PARTIAL
  (`formatDecisionMarkdown` omits provenanceIds). Duplicate pump INV-S29 → INV-S30
  (runner keeps S29). Duplicate probe P57 wallet row → P70. Leftover Discord
  message-budget prose removed after ADR 041. ARCHITECTURE tree gained pump /
  discord-wallet / host lanes. ADR 031 "no Cursor session" clause amended (ADR
  stale). Added P73–P75. Probe suite 74/74 pass via clean-context sub-agents.
  Always-on unchanged (~833 tokens). Gotchas empty.
- 2026-08-17 context-maintenance + session-learning: FOMO follows vs X review
  (ADR 048, INV-S31, schema 28). X read-only scrape allows timeline GraphQL
  POST. P10/P29 refreshed. Added P76. Graded P10/P29/P32/P76 pass via
  clean-context explore. Always-on unchanged (~833 tokens). Gotchas empty.
  `tc init` still migrates only to V14 (NOTES drift, no code change).
- 2026-08-18 context-maintenance + ADR maintenance: lint 0 errors, 2990 STE
  warnings. Always-on ~846 tokens (was ~833). ADR 033 deprecated (Discord
  budgets gone after ADR 041). ADR 026/030/032 stale prose fixed. ARCHITECTURE
  sandbox boundary matches INV-I1 PARTIAL. `tc init` now migrates to V28.
  P4/P24 failed: Phase status lived only in `ops/NOTES.md` (map skip). Fix:
  README Status + LIVE-E2E Phase status + docs/README row. Re-ran P4/P24 pass.
  Added P77 (root README entry). Sampled probes pass via clean-context explore.
  Gotchas empty. No GAP/CANDIDATE invariant rows.
- 2026-08-31 session-learning: concurrent Playwright auth DMs (INV-R6) go to
  the operator interface bot, never the router. No `ops/sync-twitter-session.sh`.
  An X hold can leave narrative-scan with an empty outbox while healthz is ok.
  Added P78. Gotchas empty.
- 2026-08-31 context-maintenance: lint 0 errors. README/NOTES said schema 28
  while code is 29. ADR 049 was missing from NOTES and the map. INV-B5 demoted
  ENFORCED to PARTIAL (dead-letter untested). INV-R6 upgraded to ENFORCED.
  Added INV-S32 (host-only discovery log / cashtag clusters). LIVE-E2E July
  launchd snapshot marked historical. Always-on 846 → 738 after live-vps
  demotion. Added P79. Sampled probes 13/13 pass via clean-context explore.
  Gotchas empty.
- 2026-09-04 context-maintenance: lint 0 errors. TECHNICAL-SPEC still said
  Fomo registers wallet candidates. INV-R2 omitted FOMO `/follows` + Statsig.
  ADR 034 still cited Discord message caps after ADR 041. smart-wallets.md
  said scans have no Cursor session. Added INV-R7. Tightened P37 so it does
  not imply Fomo writes wallets. Added P80/P81. Sampled probes 16/16 pass
  via clean-context explore. Always-on 738 → 736. Gotchas empty.
