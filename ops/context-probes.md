...
| P57 | recall | Why can wallet scans starve `outcomes-settle` / `wallet-review`, and what fixes it? How do paper track positions leave `entry-pending`? | Pre-ADR 031 scans held `agent/.lock` for Cursor+Helius; settle/review exited 3. Fix: those jobs + wallet-scan are lock-exempt with brief RMW only; scans host-only + `max_wallets_per_scan`. Entries finalise via host `settle-ledger` in `outcomes-settle` (decisionTs + first post-decision bar); drop on unfilled → `censored`. Never hand-edit `ledger.json`. → ADR 031, smart-wallets.md, INV-S10/S15 | pass | 2026-07-27 |
...
| P67 | recall | Can the pump-scan agent like or follow a coin that is not in the same-run eligible snapshot? | No. Likes bind to `pump-fyp-eligible` item ids. Follow/unfollow bind to authors in that snapshot (INV-S29). Following, leaderboard, and profile calls are evidence only → pump-fun.md, ADR 047 | pass | 2026-08-13 |
...
| P70 | recall | Do Pump profile ids enter wallets.json or the X source-call log? | No. Leaderboard and profiles never write `wallets.json` (INV-S19). Calls archive to `archive/outcomes/pump-call-*.json` only. Inbox text has no bullish words so `extractCallEvents` ignores it → ADR 047, INV-S19 | pass | 2026-08-13 |
