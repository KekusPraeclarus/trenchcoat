...
| P20 | recall | What text does Telegram vs Discord get for a market broadcast, and where do the rewrites run? | One host render path: fail-closed topic paragraph from bounded topic packet when `broadcast.telegram_overview.enabled` (`distill-session.ts` / `renderChannelPayloads`; ≤800 chars; else short `event.text`). Discord copies the same `channels.telegram.text` (`forwarded`). Grok gets a structured twin of that text (ADR 050). Topic-merged followers omit all three. `telegram_overview.daily_cap` caps LLM sessions (`topic-distill-<day>.json`). Daily `narrative.digest` is Telegram-only at 04:00 London (ADR 041). Distiller aims for ~8000 chars. Raw `.md` goes to the operator interface bot only (ADR 049) → router.md, INV-B2, ADR 049/050 | pass | 2026-09-04 |
