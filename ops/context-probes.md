...
|----|------|----------|--------------------------|-------------|------|
| P1 | recall | How do Telegram and Discord market broadcasts relate after channel render? | Same rendered text on both when `channels.telegram` is set; topic-merged followers omit both. No separate Discord daily budget (ADR 041) → router.md, orchestrator.md, INV-B2/B4 | pass | 2026-08-31 |
...
| P4 | continuation | What is the offline vs live acceptance status of the implementation? | Offline suites green. Phase 0–3 DONE 2026-07-18. No numbered Phase 4. Residual live work in `ops/LIVE-E2E-BLOCKERS.md` (also root README Status) | pass | 2026-08-31 |
| P24 | continuation | What is next after Phase 3 of the 2026-07-18 audit roadmap? | Phase 3 DONE. No numbered Phase 4. Further work is remaining PARTIAL invariants and live ops → root README Status + `ops/LIVE-E2E-BLOCKERS.md` | pass | 2026-08-31 |
...
| P20 | recall | What text does Telegram vs Discord get for a market broadcast, and where do the rewrites run? | One host render path: fail-closed topic paragraph from bounded topic packet when `broadcast.telegram_overview.enabled` (`distill-session.ts` / `renderChannelPayloads`; ≤800 chars; else short `event.text`). Discord copies the same `channels.telegram.text` (`forwarded`). Topic-merged followers omit both. `telegram_overview.daily_cap` caps LLM sessions (`topic-distill-<day>.json`). Daily `narrative.digest` is Telegram-only at 04:00 London (ADR 041). Distiller aims for ~8000 chars. Raw `.md` goes to the operator interface bot only (ADR 049) → router.md, INV-B2, ADR 049 | pass | 2026-08-31 |
...
| P73 | recall | Does INV-S6 still claim every `decisions.md` entry cites provenance ids, and is that ENFORCED? | PARTIAL. Snapshots/proposals/`checkS6` cover receipt citations. `formatDecisionMarkdown` omits `provenanceIds` from `decisions.md`. → INVARIANTS.md INV-S6, `src/orchestrator/proposals.ts` | pass | 2026-08-31 |
| P74 | artifact | Which files change when you add a pump.fun collector behaviour (engagement bind, call settle, or scrape)? | `src/collectors/pump/`, `src/social/pump-engagement.ts`, `src/orchestrator/jobs.ts` `pump-scan`, ADR 047, INV-S30, `docs/knowledge/pump-fun.md`, `docs/architecture/collectors.md` | pass | 2026-08-15 |
| P75 | continuation | What is the next step of the planned trading pipeline? | Design settled. Status NOT STARTED. No `src/trading/`, no `trade-*` jobs. Read `docs/trading/README.md` before assuming any trading code exists. | pass | 2026-08-31 |
| P76 | recall | Does following a FOMO trader write wallets.json or the X managed list? | No. Host follows on fomo.family only. State is `state/fomo-follows.json`. X nominations need an explicit FOMO profile X link. Same-handle does not enter pending. Shiller 10/5 is X-post CAs only. Do not skip the X agent only because CAs are under 10 → ADR 048, INV-S31, fomo-family.md | pass | 2026-08-31 |
| P77 | recall | Where does a human or agent start for setup, deploy, and the rest of the docs? | Root `README.md` for setup/deploy. Then `docs/README.md` for the map. `AGENTS.md` line 3. Agent section in root README. | pass | 2026-08-31 |
...
| P79 | recall | What is the current config schema, and did ADR 049 bump it? | Current schema is **29** (ADR 043 `history_days`). ADR 049 did not bump schema. `tc init` uses `migrateConfigToV29`. Triple-sync: ConfigSchema, DEPLOYMENT_CONFIG_SCHEMA, install scripts. → CONFIG.md, README setup | pass | 2026-08-31 |
