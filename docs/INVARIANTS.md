...
| INV-I3 | No credential (router/Telegram/provider keys, Twitter session material, or Cursor API keys if ever used) exists under `agent/` at any time; Cursor auth for jobs is host CLI login, never copied into the workspace | PARTIAL | secret-scan over `agent/**` + snapshot writer never receives env values; session argv built without embedding secrets |
| INV-I4 | Only the snapshot writer creates files under `agent/inbox/`, and it rejects any path resolving outside it (`..`, absolute, symlink) | PARTIAL | path guard + `prop_inv_i4_*` on `src/lib/snapshot.ts`; global "only writer" ownership not yet proven |
| INV-I5 | Outer runtime isolation: only `agent/` is mounted into the agent container; host repo, home, credentials, archive, browser profile, and sockets are absent; tool env is secret-scrubbed; `/tmp` writes and agent-tool networking are denied | PARTIAL | artifact/config tests under `tests/sandbox/`; live container smoke still open |
...
| INV-S15 | One state writer at a time: every cron job, chat research sub-agent, and recovery action holds the workspace writer lock for its full duration; lock-free chat reads never write | PARTIAL | `WorkspaceLock` (O_EXCL PID owner file) is acquired first in `runJob`; overlapping `tc run` exits 3; recovery/chat research paths do not yet take the same lock |
...
| INV-S19 | Wallet state is host-only: hard exclusions are absolute; `blended = 0.80*det + 0.20*llm` with fail-closed neutral 50; causal finalized outcomes only; score updates lag evidence; reorg/`removed` logs invalidate unfinalized rows; runtime agents cannot nominate/score/add/drop wallets | PARTIAL | unit/property tests `prop_inv_s19_*` on scoring bounds, exclusions, lag, and state ownership |
| INV-S20 | Every wallet add/drop is an immutable, idempotent transition that emits exactly one durable `wallet.lifecycle` router event with a host-rendered one-liner on a lane that does not consume market broadcast budget | GAP | `src/wallets/lifecycle.ts` builds transition/event objects only; no caller persists transitions or stages router events yet |
| INV-S21 | Source-list membership is decided only by deterministic host code from lagged, settled, direct bullish raw-CA outcomes. FYP text, model artifacts, engagement, follower counts, and same-epoch outcomes cannot promote an account. Every transition is immutable and idempotent; retries cannot produce duplicate logical membership changes | PARTIAL | transition engine + tests in `src/sources/lifecycle.ts`; production `source-list-review` still defaults to empty outcomes (`src/orchestrator/source-list.ts`) and `outcomes.ts` does not yet gate raw-CA/bullish/settled provenance |
| INV-S22 | The runtime agent owns FYP like/follow/unfollow choices for narrative/sentiment feed training. Engagement never writes managed-list membership or source scores. Likes are hard-throttled to at most 2 per 10 minutes; posts, replies, DMs, and retweets remain blocked | PARTIAL | mutation isolation in `src/social/x-engagement.ts` + `engagement.ts`; defaults are 2/10m but config allows up to 20 likes and windows as short as 1m; likes are not yet bound to collected FYP post ids |
...
| INV-B3 | Chat replies go only to allowlisted Telegram user ids | PARTIAL | sender allowlist is first in `src/chat/handler.ts` + spoofed-id tests; replies still target the inbound `chatId`, so an allowlisted operator messaging from a group can elicit a group reply |
...
| INV-B5 | Router HMAC rejects replayed nonces, skewed timestamps, and bad signatures; exact event duplicates return success without re-queue; eventId/payload conflicts return 409 and incident logs; fanout is durable at-least-once with dead-letter visibility | ENFORCED | router contract/crash tests under `src/router/**` and `tests/` |
...
| INV-R2 | Normal Twitter collectors are read-only: they never post, like, follow, repost, or DM. Host-only exceptions: (1) managed-list synchronizer may create one private list and add/remove its members after list-ID confinement; (2) engagement applicator applies the bot's like/follow/unfollow choices (FavoriteTweet / friendship ops only), with likes hard-throttled to 2 per 10 minutes. Every other mutating operation remains blocked | ENFORCED | `managed-list.ts`, `engagement.ts`; contract allowlist tests |
| INV-R3 | On HTTP 429, clients back off (honouring `Retry-After` when present) and never retry in a tight loop | PARTIAL | gated client in `src/lib/http.ts` observes 429 and honours `Retry-After` for subsequent requests; dedicated 429/retry-loop tests still open |
| INV-R4 | On-demand research (chat or CLI) goes through the same collector layer and rate gate as cron jobs — no direct upstream fetches from `src/chat/` | PARTIAL | `scripts/lint-static.ts` forbids raw chat fetches; `src/chat/` currently has none; not yet proven for every future research route |
