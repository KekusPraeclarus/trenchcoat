...
| INV-I5 | Outer runtime isolation: only `agent/` is mounted into the agent container; host repo, home, credentials, archive, browser profile, and sockets are absent; tool env is secret-scrubbed; `/tmp` writes and agent-tool networking are denied | PARTIAL | container smoke + sandbox tests under `tests/sandbox/` |
...
| INV-S15 | One state writer at a time: every cron job, chat research sub-agent, and recovery action holds the workspace writer lock for its full duration; lock-free chat reads never write | ENFORCED | flock acquisition is the run loop's first statement; concurrency test: overlapping `tc run` invocations, exactly one proceeds (other exits 3) |
...
| INV-S20 | Every wallet add/drop is an immutable, idempotent transition that emits exactly one durable `wallet.lifecycle` router event with a host-rendered one-liner on a lane that does not consume market broadcast budget | PARTIAL | transition + router staging tests; budget isolation tests |
...
| INV-B3 | Chat replies go only to allowlisted Telegram user ids | ENFORCED | allowlist check is the first statement of the message handler + unit test with spoofed ids |
