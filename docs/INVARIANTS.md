...
| INV-S12 | No model-authored artifact participates in any `sources.json` write: source↔candidate attribution is host-side string matching (contract/pair address) over the orchestrator's pre-session snapshot copies, and dock triggers come only from typed scanner responses — never from the research queue, decisions.md citations, or workspace files | GAP (pre-impl) | attribution module's inputs are typed to the host-side archive (structural once implemented); unit tests `prop_inv_s12_*` incl. a framing attempt (agent-written citation naming an innocent source must not affect scores) |
