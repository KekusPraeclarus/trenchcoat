...
| INV-S7 | `state/sources.json` is written only by deterministic host code (audit scoring maths, rug-shill dock, operator undock command) — never by any model session | GAP (pre-impl) | post-run check: `sources.json` unchanged after every agent session, else run flagged |
...
| INV-S12 | No model-authored artifact participates in any `sources.json` write, with one bounded exception: the intent classifier's verdict enters as a constrained enum that can only attenuate a dock already decided deterministically. Attribution is host-side string matching (contract/pair address) over the orchestrator's pre-session snapshot copies; dock triggers come only from typed scanner responses — never from the research queue, decisions.md citations, or workspace files | GAP (pre-impl) | attribution module's inputs are typed to the host-side archive (structural once implemented); unit tests `prop_inv_s12_*` incl. a framing attempt (agent-written citation naming an innocent source must not affect scores) |
| INV-S13 | The intent classifier is leniency-bounded and fail-closed: isolated session (no tools, no workspace state, fixed host-side prompt), output parsed strictly as `shill` \| `warn` with any other output treated as `shill`; a `warn` suspends the immediate penalty but never writes scores, and the rug-adjacency counter increments regardless of verdict | GAP (pre-impl) | unit tests `prop_inv_s13_*`: malformed/injected outputs fail to `shill`; `warn` path produces exoneration queue entry + counter increment, no score delta; red-team eval with injection-laden messages |
