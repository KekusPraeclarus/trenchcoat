...
| `farcaster-scan` | **omitted by default**; `--with-farcaster` installs ~every 4h (uniform jitter 3h15m–4h45m via `ops/run-job-jittered.sh`); **host-pre skip** `farcaster-disabled` while `farcaster.enabled` is false | Neynar for-you + optional channels + following; one trending fallback when for-you has no live evidence *(live when `farcaster.enabled`)* | trends/discovery from any usable FC feed; likes only on live for-you cast hashes (`fc-engagement.json`, ≤2 likes/10m) |
...
| `narrative-scan` | every 6h; `tc run narrative-scan --no-broadcast` merges log/INDEX memory and skips outbox, router, and research drain | sealed complete list-scan/FC archive reuse, curated into the derived social snapshots with a `narrative-evidence-quality` grade (ADR 042), + CoinGecko trending with Dex/Gecko fallback (live≤6h / stale≤24h; **degraded** when market-blind; skip if no usable evidence) | agent proposes `reports/<run-id>/narrative-proposals.jsonl` (optional monotonic `framing` maturity `rotation`→`ecosystem`/`regime` with rotation-free title); host merges into the integrity-protected `state/narratives/log.jsonl`, bridges new/peaking narratives to bounded research queue candidates, then prunes entries older than `narratives.retention_days` (default 14) and reconciles `INDEX.md` (mature lines include `framing=`); new slug, stage change, notable concrete same-stage development (incl. framing maturity), or founder/protocol primary-source catalyst → outbox. CG category ranks are confirmation only — host rejects `cg-category-list-churn` (per-category "on/off CG" / `cat #N` copy). Same-stage `narrative-emergence`/`rotation` claims are compatibility-routed through development novelty dedupe so a misclassified catalyst is not dropped; pure stage restatements and stale lane-“rotation” wording against matured subjects (`stale-narrative-framing`) remain rejected. Developments include catalysts, revenue/usage changes, material tape, identity/security risks, and names/leaders moving; founder primary-source catalysts (founder/CEO/protocol official/official channel announcing material product/wallet/protocol/ecosystem changes) must broadcast without CT-cluster or stage-shift prerequisites; recent accepted repeats are rejected. Capital-flow claim-type `rotation` remains host-rejected when market-blind (display framing ≠ claim type); single-platform rotation/sentiment-collapse is capped at `watch` and labeled `X-only` / `Farcaster-only`) |
...
| `narrative-source-review` | daily 19:00 UTC | lagged narrative-source utility. Backfills classified narrative/both first. | **no agent** — promote/demote + gated X follow |
...
5–30m delay between rounds. `farcaster-scan` is not installed by default; with
`--with-farcaster` it stays jitter-gated via `ops/run-job-jittered.sh` (uniform
[3h15m, 4h45m]). Telegram
...
2. **`archive/runs/<run-id>/broadcast-rejects.json`** (or run report reject list)
   ...
   status-quo filler, `cg-category-list-churn`), worthiness (`worthiness:…`),
   schema/ref/platform failures.
