...
|---|---|---|
| [orchestrator.md](orchestrator.md) | Job registry (incl. wallet + harness-improve / harness-meta-improve), Cursor CLI sessions, journalled/idempotent run loop, locking, outbox → router, alpha lifecycle, sealed audit epochs, ledger + source scoring, rug-dock, recovery | `src/orchestrator/`, `src/cli.ts`, `src/harness/`, `ops/` |
...
| [discord-research.md](discord-research.md) | Private-guild Discord research bot (Gateway, isolated state, watch monitor) — **not** router webhook broadcasts | `src/discord/`, `chat.discord` config (schema 16+), ADR 010, ADR 012, ADR 022 |
| [discord-wallet-signals.md](discord-wallet-signals.md) | Read-only Cielo/relay wallet-alert confluence (REST poll) — isolated from `wallets.json` | `src/collectors/discord-wallet/`, `discord-wallet-signal-scan`, ADR 035 |
| [discord-tracking.md](discord-tracking.md) | NL idea-tracking requests, durable match batches, gated research-first alerts, chain constraint, expiry, INV-D3–D8 | `src/discord/tracking-*.ts`, `chat.discord.tracking`, ADR 018, ADR 019, ADR 021 |
| [discord-conversation.md](discord-conversation.md) | Channel conversation over main workspace, addressing gate, unconfirmed research + synthesis, INV-D9 | `src/discord/conversation*.ts`, `chat.discord.conversation`, ADR 022 |
...
ADRs live under [`docs/adr/`](../adr/) (incl. 038/039/046/047). Provider knowledge under
