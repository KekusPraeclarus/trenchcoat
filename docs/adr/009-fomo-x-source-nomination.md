...
4. `narrative`/`both` enter a 14-day utility probation in
   ...
   already classified rows, including `matchBasis: same-handle`. New pending
   nominations still need an explicit FOMO profile X link. The clock starts
   at first sync, not at nominate time. Follows use the existing X engagement
