...
describe("telegram-digest schedule install", () => {
  it("registers a 04:00 Europe/London systemd calendar timer", () => {
    const script = readFileSync(
      join(process.cwd(), "ops/install-systemd.sh"),
      "utf8",
    )
    expect(script).toContain("trenchcoat-job-telegram-digest")
    expect(script).toContain('write_calendar_timer trenchcoat-job-telegram-digest "*-*-* 04:00:00 Europe/London"')
