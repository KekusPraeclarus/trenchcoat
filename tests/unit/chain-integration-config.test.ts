...
describe("config schema 18 via chain_integration migration", () => {
  it("migrates schema 11 → 18 with chain_integration defaults", () => {
    const raw = { ...seed, schema: 11 }
    delete (raw as { chat?: { discord?: { chain_integration?: unknown } } })
      .chat?.discord?.chain_integration
    const migrated = migrateConfigToV18(raw)
  ...
  it("preserves explicit enabled:false", () => {
    ...
    const parsed = ConfigSchema.parse(migrateConfigToV18(raw))
