...
import {
  ...
  STALE_VISIBLE_SEC,
} from "../../src/collectors/fomo/freshness.js"

describe("fomo freshness helpers", () => {
  ...
  it("rejects missing and future timestamps", () => {
    expect(freshnessFromIso(undefined, "2026-07-19T00:00:00.000Z").ok).toBe(false)
    expect(freshnessFromIso("2026-07-20T00:00:00.000Z", "2026-07-19T00:00:00.000Z").reason)
      .toBe("future-timestamp")
  })

  it("accepts only live events for signal snapshots", () => {
    const fetchedAt = "2026-07-19T12:00:00.000Z"
    const live = snapshotFieldsFromEvent("2026-07-19T11:00:00.000Z", fetchedAt)
    expect(live.accepted).toBe(true)
    expect(isLiveEligible(live.ageSec)).toBe(true)
    const old = snapshotFieldsFromEvent("2026-07-18T11:00:00.000Z", fetchedAt)
