import { describe, expect, it } from "vitest"
import { createHash } from "node:crypto"
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync } from "node:fs"
import { tmpdir } from "node:os"
import { join } from "node:path"
import { SnapshotWriter } from "../../src/lib/snapshot.js"
import { ConfigSchema, type TrenchcoatConfig } from "../../src/lib/config.js"
import { collectPumpScan } from "../../src/orchestrator/pump-collect.js"
import { processPumpScanEngagement } from "../../src/orchestrator/pump-engagement.js"
import { savePumpGates } from "../../src/collectors/pump/gates.js"
import { writePumpFypEligibleSnapshot } from "../../src/orchestrator/pump-fyp-eligible.js"
import type { PumpDataSource, PumpFeedItem, PumpGatesFile } from "../../src/collectors/pump/types.js"

const MINT = "EN2nnxrg8uUi6x2sJkzNPd2eT6rB9rdSoQNNaENA4RZA"
const NOW = "2026-08-13T12:00:00.000Z"
const RUN = "pump-scan-shadow-1"
const seed = JSON.parse(
  readFileSync(join(process.cwd(), "config/seed.example.json"), "utf8"),
) as Record<string, unknown>

function hashFile(path: string): string {
  return createHash("sha256").update(readFileSync(path)).digest("hex")
}

function pumpConfig(): TrenchcoatConfig {
  const base = ConfigSchema.parse(seed)
  return {
    ...base,
    pump: {
      ...base.pump,
      enabled: true,
      shadow_mode: true,
      engagement: { ...base.pump.engagement, enabled: true },
    },
  }
}

function passGates(): PumpGatesFile {
  return {
    schema: 1,
    probeRunId: "test",
    evaluatedAt: NOW,
    fixtureHashes: {},
    gates: {
      provider: { verdict: "pass", sampleSize: 30, successRate: 0.99 },
      feed: { verdict: "pass", sampleSize: 30 },
      leaderboard: { verdict: "pass", sampleSize: 30 },
      following: { verdict: "pass", sampleSize: 30 },
    },
  }
}

function item(tab: PumpFeedItem["tab"]): PumpFeedItem {
  return { itemId: `coin-${tab}`, author: `${tab}-author`, tab, mint: MINT, chain: "solana", observedAt: NOW }
}

function mockClient(): PumpDataSource {
  return {
    readFeed: async ({ tab }) => [item(tab)],
    readLeaderboard: async () => [{ handle: "lb-alpha", rank: 1, observedAt: NOW }],
    readCallerProfile: async (handle) => ({
      handle,
      calls: [{ callerId: handle, chain: "solana", tokenAddress: MINT, calledAt: NOW }],
    }),
    close: async () => undefined,
  }
}

describe("pump shadow isolation", () => {
  it("leaves wallets, queue, watchlist, and x-engagement byte-identical", async () => {
    const root = mkdtempSync(join(tmpdir(), "pump-shadow-"))
    const agentRoot = join(root, "agent")
    const archiveRoot = join(root, "archive")
    const stateDir = join(agentRoot, "state")
    mkdirSync(join(agentRoot, "inbox"), { recursive: true })
    mkdirSync(stateDir, { recursive: true })
    mkdirSync(join(agentRoot, "reports", RUN), { recursive: true })
    const wallets = join(stateDir, "wallets.json")
    const queue = join(stateDir, "research-queue.json")
    const watchlist = join(stateDir, "watchlist.json")
    const xEngagement = join(stateDir, "x-engagement.json")
    writeFileSync(wallets, `${JSON.stringify({
      schema: 1,
      wallets: [],
      transitions: [],
      pendingTransitionIds: [],
      cursors: [],
      exclusions: [],
    }, null, 2)}\n`)
    writeFileSync(queue, `${JSON.stringify({ schema: 1, entries: [] }, null, 2)}\n`)
    writeFileSync(watchlist, `${JSON.stringify({ schema: 1, entries: [] }, null, 2)}\n`)
    writeFileSync(xEngagement, `${JSON.stringify({
      schema: 1,
      followedHandles: [],
      likedPostIds: [],
      lastLikedAt: {},
      lastFollowedAt: {},
      pendingActionIds: [],
      decisions: [],
      receipts: [],
      daily: { day: "2026-08-13", likes: 0, follows: 0, unfollows: 0 },
    }, null, 2)}\n`)
    const before = {
      wallets: hashFile(wallets),
      queue: hashFile(queue),
      watchlist: hashFile(watchlist),
      xEngagement: hashFile(xEngagement),
    }
    await savePumpGates(archiveRoot, passGates())
    const writer = new SnapshotWriter(agentRoot)
    await collectPumpScan({
      runId: RUN,
      writer,
      fetchedAt: NOW,
      agentRoot,
      archiveRoot,
      config: pumpConfig(),
      sessionExists: true,
      client: mockClient(),
      cursorsPath: join(root, "cursors.json"),
    })
    await writePumpFypEligibleSnapshot({
      writer,
      runId: RUN,
      fetchedAt: NOW,
      items: [{ itemId: "coin-fyp", author: "fyp-author" }],
    })
    writeFileSync(join(agentRoot, "reports", RUN, "pump-engagement.json"), `${JSON.stringify({
      schema: 1,
      runId: RUN,
      proposedAt: NOW,
      items: [{
        action: "like",
        itemId: "coin-fyp",
        authorHandle: "fyp-author",
        reasonCode: "chart-quality",
        rationale: "clean chart",
      }],
    }, null, 2)}\n`)
    const report = await processPumpScanEngagement({
      agentRoot,
      archiveRoot,
      runId: RUN,
      nowIso: NOW,
      config: pumpConfig(),
    })
    expect(report.executed).toBe(0)
    expect(report.receipts.every((r) => r.verified === false)).toBe(true)
    expect(hashFile(wallets)).toBe(before.wallets)
    expect(hashFile(queue)).toBe(before.queue)
    expect(hashFile(watchlist)).toBe(before.watchlist)
    expect(hashFile(xEngagement)).toBe(before.xEngagement)
  })
})
