...
echo "== narrative review uses calendar timers =="
REVIEW_TIMER="$SYS_WITH/.config/systemd/user/trenchcoat-job-narrative-source-review.timer"
SCAN_TIMER="$SYS_WITH/.config/systemd/user/trenchcoat-job-fomo-narrative-source-scan.timer"
test -f "$REVIEW_TIMER"
test -f "$SCAN_TIMER"
grep -q 'OnCalendar=\*-\*-\* 19:00:00' "$REVIEW_TIMER"
grep -q 'OnCalendar=\*-\*-\* 00,06,12,18:00:00' "$SCAN_TIMER"
! grep -q OnUnitActiveSec "$REVIEW_TIMER"
! grep -q OnUnitActiveSec "$SCAN_TIMER"
grep -q 'StartCalendarInterval' "$LD_WITH/Library/LaunchAgents/com.trenchcoat.job.narrative-source-review.plist"

echo "== systemd starts timers only after deploy pause clears =="
INSTALL_SYS="$ROOT/ops/install-systemd.sh"
ENABLE_LINE="$(grep -n 'enable_unit "\$unit" timer' "$INSTALL_SYS" | head -1 | cut -d: -f1)"
CLEAR_LINE="$(grep -n '^clear_deploy_pause_and_kick$' "$INSTALL_SYS" | head -1 | cut -d: -f1)"
START_LINE="$(grep -n '^start_scheduled_timers$' "$INSTALL_SYS" | head -1 | cut -d: -f1)"
test -n "$ENABLE_LINE" && test -n "$CLEAR_LINE" && test -n "$START_LINE"
test "$ENABLE_LINE" -lt "$CLEAR_LINE"
test "$CLEAR_LINE" -lt "$START_LINE"
grep -q 'systemctl --user --no-block start "\$unit.service"' "$INSTALL_SYS"
grep -q 'enabled \$unit.timer (start after deploy pause clears)' "$INSTALL_SYS"
