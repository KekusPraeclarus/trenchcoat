...
export const AMBIENT_THREAD_GAP_MS = 15 * 60_000
...
const SUGGESTION_SIGNAL_RE = /\b(bug|fix|feature|should|please|add|improve|change|update|broken|idea|suggestion|wishlist|todo|implement|support|enable|disable|need|want|could|would|maybe|consider)\b/iu
...
export function hasSuggestionSignal(
  ...
  for (const m of messages) {
    if (SUGGESTION_SIGNAL_RE.test(m.content)) return true
    if (/<@!?\d+>/u.test(m.content)) return true
  }
  return false
}
