...
export async function runResearchPasses(args: Readonly<{
  ...
  const finalPrompt = [
    ...
    `When the dossier is solid enough to notify operators (track-worthy thesis, notable security/market finding with sealed evidence), optionally propose a market broadcast in outbox/${args.runId}.json as {schema:1,items:[{severity,text,refs,auditClaim}]} — text ≤280 chars; refs must be frozen state/… or inbox/${args.runId}/… paths. Skip outbox when thin, ambiguous, hard-fail, or nothing new. Host worthiness still gates Discord.`,
